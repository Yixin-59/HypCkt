"""Euclidean autoregressive GRU decoder for circuit DAG generation."""
...
class GRUDecoder(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        vae = cfg['vae']
        return cls(
            max_n=ds['max_n'], nvt=ds['nvt'], subn_nvt=ds['subn_nvt'],
            START_TYPE=ds['START_TYPE'], END_TYPE=ds['END_TYPE'],
            max_pos=ds.get('max_pos', 8),
            emb_dim=enc.get('emb_dim', 16),
            feat_emb_dim=enc.get('feat_emb_dim', 8),
            hs=enc.get('hs', 301),
            nz=vae.get('latent_dim', 56),
            bidirectional=enc.get('bidirectional', False),
            pos=enc.get('pos', True),
            scale=ds.get('scale', True),
            scale_factor=ds.get('scale_factor', 102),
            topo_feat_scale=enc.get('topo_feat_scale', 0.01)
        )
